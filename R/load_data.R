...
load_data <- function(){
  ...
  if(Sys.getenv("GeyseRdataPath")==""){
    path <- ifelse(readline("Would you like to set a custom path to save the data? \n Y/N \n ") %in% c("y","Y"),
  ...
  if(regmatches(time_dif,regexpr("\\d(?=\\s)",time_dif,perl = T)) != "0") {
    if(readline(paste0("Your current file is ",time_dif," old. Would you like a newer version? \n Y/N \n ")) %in% c("y","Y")){
      wow <- tempfile()
      download.file(paste0("https://geysertimes.org/archive/complete/geysertimes_eruptions_complete_",Sys.Date(),".tsv.gz"),wow)
